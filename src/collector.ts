...
function findStaleLogs(currentSessionId: string, now: number): StaleLog[] {
  let entries: string[];
  try {
  ...
  for (const entry of entries) {
    ...
    if (ageMs < STALE_LOG_MIN_AGE_MS) continue;
...
async function sweepStaleLogs(currentSessionId: string, config: ResolvedPluginConfig): Promise<void> {
...
function repoNameFromCwd(cwd: string | undefined): string | null {
  if (!cwd) return null;
...
async function processBatch(filePath: string, config: ResolvedPluginConfig): Promise<void> {
  ...
  const repo = repoNameFromCwd(
    (sessionStart?.cwd as string) || (events[0]?.cwd as string | undefined),
  );
  if (repo) {
  ...
  function handleEvent(event: Record<string, unknown>): void {
    ...
    switch (hook_event_name) {
      case "SessionStart": {
        ...
        const repo = repoNameFromCwd(event.cwd as string | undefined);
        if (repo) {
          rootSpan.setAttribute("claude_code.repo", repo);
        }
...
async function main(): Promise<void> {
  ...
  if (config.mode === "realtime") {
    ...
    if (hookEvent === "SessionStart") {
      ...
      const child = spawn(
        process.execPath,
        [fileURLToPath(import.meta.url), "--sweep", sessionId],
